...
```
GET /api/summary/by-category?scope=home&period=month
GET /api/summary/by-category?type=expense
GET /api/summary/by-category?period=year&limit=10
```

Category breakdown ranked by total amount. Groups by (category, type) so income and expense categories are listed separately.

| Param | Description |
|-------|-------------|
| `scope` | Filter by scope |
| `period` | `today`, `week`, `month` (default), `year`, `custom` |
| `from` / `to` | Required for `custom` period |
| `type` | `income` or `expense` (omit for both) |
| `source` | Filter by origin (`bank`, `cash`, `agent`, `all`) |
| `payment_method` | Filter by payment method |
...
Financial snapshots — household net worth, account balances, investments, spending by scope. Generated by Tributary's `anastasia-snapshot.js` and POSTed automatically.
...
```json
{
  "scope": "household",
  "date": "2026-03-27",
  "data": {"net_worth": {...}, "liquid_accounts": [...], "credit_cards": [...], ...}
}
```

Same (scope, date) pair replaces the existing record.

#### List / Get

```
GET /api/snapshots                       # list all
GET /api/snapshots?scope=household       # filter by scope
GET /api/snapshots?from=2026-03-01&to=2026-03-27
GET /api/snapshots/latest?scope=household
