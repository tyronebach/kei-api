...
Template-based recurring income and expenses. Instances are generated lazily (no background job). Works for rent, utilities, subscriptions, salary — any regular cash flow in either direction.

**Frequencies:** `monthly` | `weekly` | `biweekly` | `yearly` | `custom` (interval = days)

```
POST   /api/recurring                          Create a rule
GET    /api/recurring                          List rules (+ next_due computed)
GET    /api/recurring/{id}                     Get rule
PATCH  /api/recurring/{id}                     Edit in place (all future unmodified instances follow)
PATCH  /api/recurring/{id}?effective_from=DATE Fork: close old rule, create new from date
POST   /api/recurring/{id}/stop?end_date=DATE  Stop (defaults to today)
POST   /api/recurring/{id}/skip?skip_date=DATE Skip one occurrence
DELETE /api/recurring/{id}/skip/{date}         Restore a skipped occurrence
GET    /api/recurring/{id}/instances?from=&to= List occurrences (projected/confirmed/skipped)
POST   /api/recurring/{id}/generate?through=   Materialise projected instances as transactions
DELETE /api/recurring/{id}                     Soft delete (existing transactions kept)
```
...
**Income rule (tenant paying you rent):**
```json
POST /api/recurring
{
  "scope": "home",
...
**View instances for H1 2026:**
```
GET /api/recurring/{id}/instances?from=2026-01-01&to=2026-06-30
→ [
    {"rule_date": "2026-01-01", "status": "confirmed", "transaction_id": "abc...", "amount": 2000},
    {"rule_date": "2026-02-01", "status": "confirmed", "transaction_id": "def...", "amount": 2000},
    {"rule_date": "2026-03-01", "status": "skipped",   "transaction_id": null,    "amount": 2000},
    {"rule_date": "2026-04-01", "status": "projected", "transaction_id": null,    "amount": 2000},
    ...
  ]
```

**Change rent from April 1st (fork):**
```
PATCH /api/recurring/{id}?effective_from=2026-04-01
{"amount": 2200, "name": "Rent (new rate)"}
→ closes old rule at 2026-03-31, returns new rule starting 2026-04-01
```
...
| Intent | Action |
|---|---|
| Edit just one month | `PUT /api/transactions/{id}` (existing endpoint) |
| Change going forward | `PATCH /api/recurring/{id}?effective_from=DATE` |
| Fix all future (unmodified) | `PATCH /api/recurring/{id}` (in-place) |
| Stop subscription | `POST /api/recurring/{id}/stop` |
| Skip one month | `POST /api/recurring/{id}/skip?skip_date=DATE` |
...
kei -s home recurring add expense 19.99 subscriptions --name Netflix
...
# Materialise as real transactions
